...
import { serializeTokens } from '@pancakeswap/tokens'
...
export function useGasPrice(): string {
  ...
  const { data } = useFeeData({
    chainId,
    enabled: chainId !== ChainId.PULSE && chainId !== ChainId.BSC_TESTNET,
    watch: true,
  })
...
export function useTrackedTokenPairs(): [Token, Token][] {
  ...
  const { data: farmPairs = [] } = useSWRImmutable(chainId && ['track-farms-pairs', chainId], async () => {
    ...
    const fPairs: [Token, Token][] = farms
      ...
      .map((farm) => [serializeTokens(farm.token), serializeTokens(farm.quoteToken)])
  ...
  const userPairs: [Token, Token][] = useMemo(() => {
    ...
    return Object.keys(forChain).map((pairId) => {
      return [serializeTokens(forChain[pairId].token0), serializeTokens(forChain[pairId].token1)]
    })
