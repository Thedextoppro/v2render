    ...
    <>
      ...
      <Text ml="5px" small>
        *{t('Base APR (DEX yield only)')}:{' '}
        {strikethrough ? (
          <Text style={{ display: 'inline-block' }} color="secondary" small>{`${(apr * boostMultiplier).toFixed(
            2,
          )}%`}</Text>
        ) : (
          `${apr.toFixed(2)}%`
      ...
      {strikethrough && <Text color="secondary">{t('Boost only applies to base APR (DEX yield)')}</Text>}
    </>,
    {
      placement: 'top',
    },
