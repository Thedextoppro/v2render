...
import { Flex, Heading, Card, Box, Text, CardBody, CardFooter, Button, useToast } from '@pancakeswap/uikit'
...
import { useBurnContract } from 'hooks/useContract'
  ...
  const handleBurn = useCallback(
    async () => {
      const receipt = await fetchWithCatchTxError(() => {
        ...
        return callWithGasPrice(burnContract, "convertLps", [token0s, token1s])
      })
      if (receipt?.status) {
        toastSuccess(
    ...
    [callWithGasPrice, fetchWithCatchTxError, t, toastSuccess, burnContract, token0s, token1s],
  )
            ...
            <Card p="0px" style={{ zIndex: 1 }}>
              ...
              <StyledCardFooter>
                {!account ?
                  ...
                  <Box>
                    <Button width={"100%"} onClick={async() => await handleBurn()}>Burn</Button>
                  </Box>}
              </StyledCardFooter>
            </Card>
          </CardWrapper>
