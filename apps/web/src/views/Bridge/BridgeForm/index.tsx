...
const getPID = (symbol?: string, chainId?: number) => {
  ...
  if (symbol === 'KNB') {
    return chainId === ChainId.PULSE ? { pid: 1, isNative: true } : { pid: 1, isNative: false }
  }
  if (symbol === 'DEX') {
...
export const BridgeForm: React.FC<Record<string, never>> = () => {
  ...
  const sourceChain = chainId === ChainId.ETHEREUM ? chainId : ChainId.PULSE
  const targetChain = sourceChain === ChainId.ETHEREUM ? ChainId.PULSE : ChainId.ETHEREUM
  ...
  } = useDerivedBridgeInfoWithStableSwap(
    independentField,
    typedValue,
    inputCurrency,
    outputCurrency,
    poolInfo?.min.toString(),
    poolInfo?.enabled,
  )
          ...
          <CurrencyInputPanelForBridge
            ...
            value={
              formattedAmounts[Field.INPUT]
                ? ((Number(formattedAmounts[Field.INPUT]) * (10000 - fee)) / 10000).toString()
                : formattedAmounts[Field.INPUT]
            }
            showMaxButton={false}
            maxAmount={maxAmountInput}
