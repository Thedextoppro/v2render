...
function CurrencySearchBridge({
  ...
  const aTokens = Object.values(allTokens).filter(
    (token) => ['KNB', 'MATIC', 'DEX', 'USDT'].filter((s) => s === token.symbol).length > 0,
  )
  ...
  const filteredTokens: Token[] = useMemo(() => {
    ...
    return aTokens.filter(filterToken)
  }, [aTokens, debouncedQuery])
