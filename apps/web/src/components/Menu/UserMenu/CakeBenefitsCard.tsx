...
const CakeBenefitsCard: React.FC<React.PropsWithChildren<CakeBenefitsCardProps>> = ({ onDismiss }) => {
  ...
  const {
    ...
    <>
      <Text>
        {t(`%lockedCake% DEX (including rewards) are locked in the DEX Pool until %lockedEndTime%`, {
  ...
  const {
    ...
    <>
      <Text>
        {t(`iCAKE allows you to participate in the IFO public sales and commit up to %iCake% amount of DEX.`, {
        ...
        <>
          ...
          <Message mt="8px" mb="16px" variant="warning">
            <MessageText maxWidth="200px">
              {t(
                'Lock DEX to enjoy the benefits of farm yield boosting, participating in IFOs, voting power boosts, and so much more!',
              )}{' '}
              <NextLinkFromReactRouter
